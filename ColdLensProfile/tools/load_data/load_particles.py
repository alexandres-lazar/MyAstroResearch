...
def load_particles(simPath: str, snapNum: int = 600, partType: str = "PartType4", 
                   key_list=None, hubble: float = 1.0, scale_factor: float = 1.0,
                   coord_conv: float = 1.0, *args, **kwargs) -> None:
    """Load relavent quantities of simulation particle data based on 
       specified particle type and associated keys
    """
    results = dict()
    
    print(f"... Fetching {partType} data from snapshot {snapNum}")
    ...
    for key, values in results.items():
        if key == "Masses":
            ...
            results[key] *= coord_conv * scale_factor / hubble # convert to physical kpc
